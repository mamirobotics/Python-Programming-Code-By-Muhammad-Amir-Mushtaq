...
def preprocess(data):
    for row in data:
        if isinstance(row[4], str):  # Convert 'Pat' column
            row[4] = pat_map[row[4]]
        if isinstance(row[5], str):  # Convert 'Price' column only if it's not already converted
            row[5] = price_map[row[5]]
        if isinstance(row[9], str):  # Convert 'Est' column
            row[9] = est_map[row[9]]
        if isinstance(row[8], str):  # Convert 'Type' column
            row[8] = type_map[row[8]]
